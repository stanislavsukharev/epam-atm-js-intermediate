...
export class CloudStoragePage extends BasePage {
  ...
  async fillEstimateFields({
    ...
    await this.page.getByRole('spinbutton', { name: 'Monthly data written tooltip' }).fill(dataWritten);
    await this.page.getByRole('spinbutton', { name: 'Monthly data read within' }).fill(dataRead);
    await this.page.getByRole('spinbutton', { name: 'Data retrieval amount tooltip' }).fill(dataRetrieved);
    await this.page.getByRole('spinbutton', { name: 'Data Transfer within Google' }).fill(dataTransferred);
  }
  ...
  validateCsvCost(rows: Record<string, string>[], expectedCost: string): void {
    ...
    if (!actualCost) throw new Error('"total_price, USD" field is missing or empty in the total row.');
