...
export class CloudStoragePage extends BasePage {
  ...
  async fillEstimateFields(fields: {
    totalStorage: string;
    dataWritten: string;
    dataRead: string;
    ...
    await this.totalAmountOfStorage.fill(fields.totalStorage);
    await this.page.getByRole('spinbutton', { name: 'Monthly data written tooltip' }).fill(fields.dataWritten);
    await this.page.getByRole('spinbutton', { name: 'Monthly data read within' }).fill(fields.dataRead);
    await this.page.getByRole('spinbutton', { name: 'Data retrieval amount tooltip' }).fill(fields.dataRetrieved);
    await this.page.getByRole('spinbutton', { name: 'Data Transfer within Google' }).fill(fields.dataTransferred);
  }
  ...
  async downloadEstimateAsCsv(): Promise<string> {
    ...
    const downloadButton = this.page.locator('button[aria-label="Download estimate as .csv"]');
